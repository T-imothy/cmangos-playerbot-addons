...
if Mangosbot_Original_ChatFrame_OnEvent then
    ChatFrame_OnEvent = function(self, event, ...)
        ...
        if event == "CHAT_MSG_PARTY" and mangosbot_options and
           ...
           message == MangosbotLastSuppressedPartyText and
           sender == UnitName("player") and
           (GetTime() - MangosbotLastSuppressedPartyTime) < 3 then
            return
        end

        -- WotLK ChatFrame_OnEvent requires the frame, event and the
        -- complete original event payload. Never drop the message args.
        return Mangosbot_Original_ChatFrame_OnEvent(self, event, ...)
    end
end
...
function SendBotCommand(text, chat, lang, channel)
    ...
    if (chat == "SAY") then
        ...
        -- WotLK 3.3.5a supports the addon-message transport used by the
        -- existing working CMaNGOS WotLK Mangosbot addon, including WHISPER.
...
local CMaNGOSToolbarLabels = {
    ...
    CLASS_DEATHKNIGHT = "Death Knight Role and Abilities",
...
local function CMaNGOSFriendlyClass(class)
    ...
    if class == "DEATHKNIGHT" then return "Death Knight Controls" end
...
function CMaNGOSApplySelectedBotLayout(frame, class)
    ...
    for tabName, button in pairs(frame.mantech.tabs) do
        ...
        if tabName == frame.mantech.currentTab then
            if fontString then fontString:SetTextColor(1.0, 0.82, 0.25) end
            if normalTexture then normalTexture:SetVertexColor(0.72, 0.12, 0.08) end
        else
            if fontString then fontString:SetTextColor(0.80, 0.80, 0.80) end
            if normalTexture then normalTexture:SetVertexColor(0.35, 0.35, 0.35) end
        end
    end
...
function CreateSelectedBotPanel()
    ...
    CreateToolBar(frame, -y, "CLASS_DEATHKNIGHT", {
        ["blood"] = {
            icon = "tank",
            icon_native = "spell_deathknight_bloodpresence",
            command = {[0] = "#a co +blood,+tank assist,+close,+pull,-frost,-unholy,-dps assist,-ranged,?"},
            strategy = "blood",
            tooltip = "Blood mode (tank)",
            index = 0
        },
        ["frost"] = {
            icon = "frost",
            icon_native = "spell_deathknight_frostpresence",
            command = {[0] = "#a co +frost,+dps assist,+close,-blood,-unholy,-tank assist,-pull,?"},
            strategy = "frost",
            tooltip = "Frost mode (melee DPS)",
            index = 1
        },
        ["unholy"] = {
            icon = "shadow",
            icon_native = "spell_deathknight_unholypresence",
            command = {[0] = "#a co +unholy,+dps assist,+close,-blood,-frost,-tank assist,-pull,?"},
            strategy = "unholy",
            tooltip = "Unholy mode (melee DPS)",
            index = 2
        },
        ["aoe"] = {
            icon = "caster_aoe",
            icon_native = "spell_shadow_deathanddecay",
            command = {[0] = "#a co ~frost aoe,~unholy aoe,?"},
            strategy = "aoe",
            tooltip = "Toggle Death Knight AOE behavior",
            index = 3
        },
        ["bdps"] = {
            icon = "boost",
            icon_native = "spell_deathknight_classicon",
            command = {[0] = "#a co ~bdps,?"},
            strategy = "bdps",
            tooltip = "Death Knight combat buff behavior",
            index = 4
        }
    })
...
local CMaNGOSClassData = {
    DEATHKNIGHT = {
        { label="ROLE / SPEC", buttons={
            {key="dk_blood",icon="tank",tip="Blood tank mode",commands={"#a co +blood,+tank assist,+close,+pull,-frost,-unholy,-dps assist,-ranged,?"}},
            {key="dk_frost",icon="frost",tip="Frost melee DPS mode",commands={"#a co +frost,+dps assist,+close,-blood,-unholy,-tank assist,-pull,?"}},
            {key="dk_unholy",icon="shadow",tip="Unholy melee DPS mode",commands={"#a co +unholy,+dps assist,+close,-blood,-frost,-tank assist,-pull,?"}},
        }},
        { label="DEATH KNIGHT BEHAVIOR", buttons={
            {key="dk_frostaoe",icon="caster_aoe",tip="Toggle Frost DK AOE strategy",commands={"#a co ~frost aoe,?"}},
            {key="dk_unholyaoe",icon="aoe",tip="Toggle Unholy DK AOE strategy",commands={"#a co ~unholy aoe,?"}},
            {key="dk_bdps",icon="boost",tip="Toggle Death Knight combat buff strategy",commands={"#a co ~bdps,?"}},
            {key="dk_pull",icon="pull",tip="Toggle DK pull behavior (uses Icy Touch)",commands={"#a co ~pull,?"}},
        }}
    },
...
CMaNGOSAdvancedPartyBots = function()
    ...
    if GetNumRaidMembers and GetNumRaidMembers() > 0 then
        ...
        -- In a five-player WotLK party, the player is `player` and the
...
function CMaNGOSAdvancedRender(frame)
    ...
    for name,button in pairs(frame.tabs) do
        if name==tab then
            do local fs = button:GetFontString(); if fs then fs:SetTextColor(1,.82,.25) end end
            if button:GetNormalTexture() then button:GetNormalTexture():SetVertexColor(.72,.12,.08) end
        else
            do local fs = button:GetFontString(); if fs then fs:SetTextColor(.80,.80,.80) end end
...
local function CreateIntegratedBotBar()
    ...
    for i = 1, count do
        ...
        if not icon then
            -- Some legacy clients do not expose the ActionButtonTemplate icon
...
local CMaNGOSHelpPages = {
    {
        ...
        text =
            ...
            "Mangosbot is a visual controller for CMaNGOS WotLK PlayerBot commands. It does not replace the bot AI; it gives you faster access to commands and strategies already supported by your server.\n\n|cffffcc00WotLK expansion support|r\nWotLK adds Death Knights, Isle of Conquest, and the TBC-era Arena/Eye of the Storm bot behavior. The PlayerBot core also includes WotLK glyph-management behavior as a non-combat AI feature.\n\nSelecting a Death Knight in CLASS loads Blood, Frost and Unholy controls plus DK-specific AOE, buff and pull options. Blood is presented as tank-oriented; Frost and Unholy as melee DPS-oriented.\n\n" ..
    ...
    },
    {
        ...
        text =
            ...
            "|cffffcc00CLASS|r - Only controls relevant to that bot's class/spec, including applicable AoE, buff, cure, pet, poison, totem, aura/blessing and healing options. Death Knights receive Blood, Frost and Unholy controls with DK-specific AOE/buff/pull options.\n\n" ..
    ...
    },
    {
        ...
        text =
            ...
            "|cffffcc00CLASS / CHARACTER|r - Select a current party/raid member from the dropdown; CLASS sends class strategies to that selected name, while CHARACTER uses explicit .bot setup commands.\n\n|cffffcc00WotLK-only additions|r - Death Knight CLASS controls are included. The core also supports Isle of Conquest and retains Arena/Eye of the Storm behavior. Glyph management exists as a WotLK PlayerBot non-combat behavior; this addon does not expose an unverified manual Glyph command."
    },
    {
        key = "ROLES",
        title = "Roles & Pulling",
    ...
    },
    {
        ...
        text =
            ...
            "RPG modes include general NPC interaction plus Quest, Vendor, Explore, Maintenance, Player, Craft and Battleground behavior.\n\n|cffffcc00WotLK PvP|r\nWotLK retains Warsong Gulch, Arathi Basin, Alterac Valley, Eye of the Storm and Arena behavior and adds Isle of Conquest support.\n\n|cffffcc00Glyphs|r\nThe WotLK PlayerBot core includes glyph-management behavior as part of its non-combat AI. Mangosbot leaves that core behavior automatic; there is currently no manual Glyph button because an external chat command has not been confirmed for this build.\n\n" ..
...
local function CMaNGOSHelpRender(frame, key)
    ...
    for name,button in pairs(frame.pageButtons) do
        if name == page.key then
            ...
            do local fs = button:GetFontString(); if fs then fs:SetTextColor(.9,.9,.9) end end
        end
    end
end
...
function CreateMangosbotHelpWindow()
    ...
    local subtitle = f:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    subtitle:SetPoint("TOPLEFT", title, "BOTTOMLEFT", 0, -3)
    subtitle:SetText("|cff9ca9bdCMaNGOS WotLK PlayerBot Manager  |  Designed by Tim|r")
